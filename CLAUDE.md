...
**The same reasoning is why a second job did not get a second `claude_query`.** What differs between jobs is only the *shape of the deliverable* — a topic summary vs. a skim of a community page — so `build_command()` / `ask()` take a `system_prompt` and everything else (command assembly, model/effort resolution, JSON handling, `fix_bullets`) is shared. Copying `build_command()` per job would mean one copy gets fixed and the rest silently drift — the same failure the imported alias table avoids.

**The prompt itself, though, lives in the job, and `claude_query` has no default for it.** The deliverable's shape *is* the job's identity, and everything the shape is derived from — the header `render_document()` writes, the filename prefix, the advice `warn_if_too_long()` prints — already sits in the job module; a prompt parked one module away is a prompt nobody re-reads when they change the header. So `daily_briefing.SYSTEM_PROMPT` and `url_briefing.SYSTEM_PROMPT` are peers, and `system_prompt` is a **required** parameter on both `build_command()` and `ask()`. A default would be the worse failure of the two available: a job that forgot its prompt would quietly render in the *other* job's shape, detectable only by reading the output, whereas a required argument fails at the call with `TypeError`.

**Both jobs deliver the same shape, on purpose:** an overview sentence or two, a blank line, then exactly 6 `-` bullets whose title is a Discord masked link (`- [__"제목"__](<주소>) 설명`). The two jobs' notifications land side by side in one channel, so a reader should learn one rule, not two. The link syntax's three pieces are explained once, in `url_briefing.SYSTEM_PROMPT` — `daily_briefing.SYSTEM_PROMPT` points at it rather than restating it.

**The summary length is derived from the Discord budget, not from taste — and the derivation assumes one topic per run.** `daily_briefing.SYSTEM_PROMPT` demands a ≤150-character overview and 6 bullets with an ≤80-character description each. The arithmetic: 2000 (message body) − ~70 (header: title, timestamp, topic count, `## 주제`) − 9 (truncation notice) = 1921 for the body, and 152 + 6 × 191 ≈ 1290 → a ~1360-character document, **47% of headroom** (measured, not estimated; worst case 제목 40 / 주소 90 → 1569). A bullet's 191 breaks down the same way as `url_briefing`'s (itemized two paragraphs below). Headroom is larger here than there (41%) for one reason: this job's header names topics, so it carries no target URL.

**The masked link replaced the parenthesized outlet name (`(연합뉴스)`), and that is a swap, not an addition.** The outlet name was only ever a stand-in for a source URL there was no budget to print; now that the title itself leads to the article, naming the outlet again is duplication — one click shows it. What paid for the link is the description, cut from 200 characters to 80. As in `url_briefing`, masking a URL *costs* 13 characters rather than saving any: the limit counts the raw content, not what the reader sees.

**Raising the topic count breaks this arithmetic.** At 191 characters a bullet, the second topic is cut starting from its own overview, and later topics lose even their heading (the file still has everything). Scaling up means re-dividing the prompt's allowance by the topic count — roughly 3 bullets × 40-character descriptions at three topics. One topic per cron line is the better answer, since that also gets each topic its own notification.
