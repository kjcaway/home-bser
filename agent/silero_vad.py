"""Silero VAD 래퍼 (프레임별 음성/무음 판정).

발화 종료 감지(endpointing)에 사용한다. 고정 길이 녹음을 대체하는
audio_record.record_until_silence() 가 이 모듈의 SileroVAD 로 매 프레임을 판정한다.
...
class SileroVAD:
    ...
    모델 로드는 팩토리 메서드 SileroVAD.load() 로 한다. 이 파일에는 클래스 하나만
    두는 규칙(CLAUDE.md "Python 코드 규칙")이라 모듈 함수 load_vad() 를 둘 수 없고,
    로더를 남의 모듈로 보내면 '어디서 만드는지'가 흩어지기 때문이다.
    """
    ...
    @classmethod
    def load(cls, threshold=VAD_THRESHOLD):
        """번들된 jit 모델을 로드해(네트워크 없이) 인스턴스를 만든다."""
        from silero_vad import load_silero_vad
        return cls(load_silero_vad(), threshold)
