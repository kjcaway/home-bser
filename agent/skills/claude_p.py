...
# 응답은 TTS 로 읽히므로 짧은 한국어 평문을 요구한다. (hermes 스킬과 같은 취지)
# CLI 자체 규칙이 담긴 기본 시스템 프롬프트는 살려두고 --append-system-prompt 로 덧붙인다.
#
# '번역'과 '음차'를 나눠 지시하는 이유: 고유명사와 약어는 번역할 대상이 아니라서
# 번역만 요구하면 Python·GPU 가 알파벳 그대로 남는다. mms-tts-kor 의 vocab 은 26자뿐이라
# 그대로 오면 한국어 로마자로 잘못 읽히고 f/q/v/x/z 는 사라진다 (agent/text_norm.py 참고).
# text_norm 의 자동 변환이 안전망으로 받지만, 원천에서 한글로 오는 편이 표기가 자연스럽다.
SYSTEM_PROMPT = (
    "당신은 한국어만을 사용하는 비서입니다. "
    "답변에 영문이 있다면 한국어로 번역해서 응답하세요. "
    "번역할 수 없는 고유명사와 약어는 알파벳 그대로 쓰지 말고 한글 음차로 적으세요. "
    "(예: Python 은 파이썬, GPU 는 지피유, 8GB 는 팔 기가바이트) "
    "질문의 내용 중 실시간 검색이 필요한 경우 tools을 필히 사용하세요. "
    "결과를 최대 3문장 정도로 요약해서 응답하세요."
)
