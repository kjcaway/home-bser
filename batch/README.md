...
> 생성: 2026-07-31 07:00:12 · 모델: claude-sonnet-5 · effort: high
> 대상: https://example.com/board
...
`batch/url_briefing.py`(URL 브리핑) — **개요 150자 + 항목 6개 · 항목당 180자 · URL 금지**

```
본문 상한 2000자 − 머리말(제목·생성 정보·대상 URL) ~200자 − 잘림 표시 9자 = 1791자
  개요 150자 + 항목 6개 × 183자 ≈ 1248자 → 문서 전체 ~1395자  ✅ (여유 605자)
```

항목당 180자로 20자 짧은 것은 머리말에 **대상 URL 한 줄이 더 들어가기** 때문입니다.
`- 항목` 안에 URL 을 금지하는 이유도 같은 예산인데 이쪽이 더 급합니다 — 커뮤니티 게시글
주소는 쿼리 문자열이 붙어 100자를 넘기 일쑤라, 여섯 개면 항목 서너 개 분량을 먹습니다.


모델이 지시보다 길게 쓸 때 언제 잘리는지 (정기 요약 기준, URL 브리핑도 43% 지점으로 거의 같음).
