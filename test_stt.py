...
동작:
  Enter 를 누르면 VAD 동적 녹음(발화가 끝날 때까지)을 한 뒤 텍스트로 변환해 출력한다.
  실제 에이전트와 동일하게 record_until_silence 로 녹음하므로, 고정 길이가 아니라
  말이 끝나면 자동으로 종료된다(발화가 없으면 start-timeout 후 조용히 넘어감).
...
def main():
    ...
    # ==========================================
    # 2. STT 모델 + VAD 로드 (웨이크워드/TTS 는 로드하지 않음)
    # ==========================================
    ...
    print("\n====================================================")
    print("🎙️  [STT 정확도 테스트]")
    print("    Enter=녹음 시작(말이 끝나면 자동 종료) / 'q'+Enter=종료")
    print("====================================================\n")
    ...
    try:
        while True:
            ...
            input("👉 Enter 를 누르면 녹음을 시작합니다 (말이 끝나면 자동 종료)...")

            # 녹음 직전 버퍼에 남은 오래된 오디오를 비워 녹음 창을 정렬한다.
            flush_input_stream(stream)

            print("🔴 녹음 중... (말씀하세요)")
            # 실제 에이전트와 동일하게 VAD 로 발화가 끝날 때까지 동적 녹음
            ...
            if not pcm_bytes:
                # 발화가 감지되지 않음 → 이번 측정은 건너뛴다(통계에도 반영 안 함)
                print("🤫 발화가 감지되지 않았습니다. 다시 시도하세요.")
                print("----------------------------------------------------\n")
                continue
